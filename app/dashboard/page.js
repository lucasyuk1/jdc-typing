...
const ultimos10 = resultados.slice(0, 10).reverse(); // últimos 10 resultados em ordem cronológica

const labels = ultimos10.map(r => new Date(r.created_at).toLocaleDateString("pt-BR"));
...
{/* HEADER */} <header className="flex justify-between items-center mb-10"> <div> <h1 className="text-4xl font-bold">Olá, {user.username}!</h1> <p className="text-gray-400 mt-1">
Turma: <span className="font-semibold text-blue-300">{user.turma}</span> • Idade: <b>{user.idade}</b> </p> </div> <div className="flex items-center gap-4"> <Image src={Mascote} alt="Mascote" width={60} height={60} /> <nav className="flex gap-4"> <a href="/test" className="link-nav">Fazer Teste</a> <a href="/ranking" className="link-nav">Ranking</a> <button onClick={handleLogout} className="btn-logout">Sair</button> </nav> </div> </header>
