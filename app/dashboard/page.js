...
async function loadResultados() {
  const res = await fetch("/api/results/ranking", {
    ...
    body: JSON.stringify({ mode: "pessoal", username: user.username, turma: user.turma }),
  });
...
const labels = resultados.map(r => new Date(r.created_at).toLocaleDateString("pt-BR")).reverse();
...
<p style={{ color: "#ccc" }}>Turma: <b>{user.turma}</b> | Idade: <b>{user.idade}</b></p> </div> <nav> <a href="/test" style={linkStyle}>Fazer teste</a> <a href="/ranking" style={linkStyle}>Ranking</a> <button onClick={handleLogout} style={buttonStyleLogout}>Sair</button> </nav> </div>
  ...
  <div style={{ ...cardStyle, display: "flex", flexDirection: "column", alignItems: "center", gap: 10 }}>
    ...
    <p style={{ fontSize: 48, fontWeight: "bold", color: "#4a90e2", margin: 0, textAlign: "center" }}>{mediaWPM ?? "—"}</p>
  </div>
...
const buttonStyleLogout = { marginLeft: 20, padding: "8px 16px", borderRadius: 8, border: "none", cursor: "pointer", background: "#ef4444", color: "#fff", fontWeight: "bold" };
