...
// Função de agrupamento reutilizada (média por usuário)
...
// Função para calcular média geral de WPM
function calcularMedia(lista) {
  if (lista.length === 0) return 0;
  const soma = lista.reduce((acc, r) => acc + r.wpm, 0);
  return Math.round(soma / lista.length);
}

let resultadoFinal = [];
if (mode === "pessoal" && usuario_id) {
  let pessoal = rows.filter((r) => String(r.usuario_id) === String(usuario_id));
  resultadoFinal = agrupar(pessoal).sort((a, b) => b.wpm - a.wpm);
} else if (mode === "turma" && turma) {
  const filtrado = rows.filter((r) => r.turma === turma);
  resultadoFinal = agrupar(filtrado).sort((a, b) => b.wpm - a.wpm);
} else {
  resultadoFinal = agrupar(rows).sort((a, b) => b.wpm - a.wpm);
}

const mediaGeral = calcularMedia(resultadoFinal);

return new Response(
  JSON.stringify({ success: true, data: resultadoFinal, avgWPM: mediaGeral }),
  { status: 200 }
);
...
return new Response(
JSON.stringify({ success: false, data: [], avgWPM: 0 }),
{ status: 500 }
);
}
}
